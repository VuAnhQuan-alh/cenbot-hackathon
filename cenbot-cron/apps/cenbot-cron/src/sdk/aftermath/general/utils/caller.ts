...
export class Caller {
  ...
  constructor(
    ...
    this.apiBaseUrl =
      network === undefined ? undefined : Caller.apiBaseUrlForNetwork(network);
  }
  ...
  protected async fetchApi<Output, BodyType = undefined>(
    ...
    // @ts-ignore
    const response = await Caller.fetchResponseToType<Output>(uncastResponse);
    return response;
  }
  ...
  protected async fetchApiTransaction<BodyType = undefined>(
    ...
    return TransactionBlock.from(
      await this.fetchApi<SerializedTransaction, BodyType>(url, body, signal),
    );
  }
