...
export class CoinApi {
  ...
  private createLpCoinMetadata = async (inputs: {
    ...
    try {
      ...
      const poolObjectId =
        await PoolsApi.fetchPoolObjectIdForLpCoinType(inputs);
      ...
      const coinDescription = await PoolsApi.createLpCoinMetadataDescription({
        poolName: pool.name,
  ...
  private static coinWithAmountTx = (inputs: {
    ...
    if (coinObjectIds.length > 1) {
      tx.add({
        ...
        sources: [...coinObjectIds.slice(1).map((coinId) => tx.object(coinId))],
      });
    }
