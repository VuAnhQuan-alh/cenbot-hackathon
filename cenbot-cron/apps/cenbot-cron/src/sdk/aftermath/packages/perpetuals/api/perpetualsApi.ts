...
export class PerpetualsApi {
  ...
  public fetchAccount = async (inputs: {
    ...
    const accountDfInfos =
      await this.Provider.DynamicFields().fetchAllDynamicFieldsOfType({
        parentObjectId:
          this.addresses.perpetuals.objects.exchanges[inputs.collateralCoinType]
            ?.accountManager!,
      });

    const accountDfInfo = accountDfInfos.find((info) => {
      return BigInt((info.name.value as any).account_id) === inputs.accountId;
    })!;
  ...
  public fetchPositionOrderDatas = async (
    ...
    const { askOrderIds, bidOrderIds } =
      await this.fetchPositionOrderIds(inputs);
    ...
    for (const [index, event] of eventsData.events.entries()) {
      ...
      eventsData.events[index].collateralDelta =
        Casting.IFixed.iFixedFromNumber(
          Math.abs(Casting.IFixed.numberFromIFixed(event.collateral)) -
            Math.abs(Casting.IFixed.numberFromIFixed(previousEvent.collateral)),
        );
    }
  ...
  public fetchPreviewOrder = async (
    ...
    const { collateralCoinType, marketId, side, lotSize, tickSize } = inputs;
    ...
    try {
      ...
      const postedOrderEvents =
        Aftermath.helpers.events.findCastEventsOrUndefined({
          ...
          castFunction: Casting.perpetuals.postedOrderEventFromOnChain,
        });
      ...
      const priceSlippage = !bestOrderbookPriceBeforeOrder
        ? 0
        : Math.abs(
            bestOrderbookPriceBeforeOrder - bestOrderbookPriceAfterOrder,
          );
      const percentSlippage = !bestOrderbookPriceBeforeOrder
        ? 0
        : priceSlippage / bestOrderbookPriceBeforeOrder;
  ...
  public fetchOrderbookPrice = async (inputs: {
    ...
    const bytes = await this.Provider.Inspections().fetchFirstBytesFromTxOutput(
      {
        tx,
      },
    );
  ...
  public cancelOrderTx = (inputs: {
    ...
    const { tx, collateralCoinType, accountCapId, marketId, side, orderId } =
      inputs;
    const exchangeCfg =
      this.addresses.perpetuals.objects.exchanges[collateralCoinType]!;
    return tx.moveCall({
  ...
  public placeSLTPOrderTx = (
    ...
    const orderPrice = 'price' in inputs ? inputs.price : inputs.marketPrice;
  ...
  public bookPriceTx = (inputs: {
    ...
    return tx.moveCall({
      ...
      typeArguments: [],
      arguments: [
        typeof orderbookId === 'string' ? tx.object(orderbookId) : orderbookId, // Orderbook
      ],
    });
  };
  ...
  public inspectPlaceLimitOrderTx = (inputs: {
    ...
    const { tx, orderbookId, accountId, side, size, price, orderType } = inputs;
    return tx.moveCall({
      target: Helpers.transactions.createTxTarget(
        this.addresses.perpetuals.packages.perpetuals,
        PerpetualsApi.constants.moduleNames.orderbook,
  ...
  public fetchExecutionPrice = async (
    ...
    const executionPrice = fillReceipts.reduce((acc, receipt) => {
      ...
      return acc + orderPriceNum * (Number(receipt.size) / Number(sizeFilled));
    }, 0);
  ...
  public static bucketOrders = (inputs: {
    ...
    let dataPoints: OrderbookDataPoint[] = orders.reduce(
      (acc, order) => {
        ...
        const placementIndex = acc.findIndex((dataPoint: OrderbookDataPoint) =>
          side === PerpetualsOrderSide.Ask
    ...
    for (const [index, data] of dataPoints.entries()) {
      dataPoints[index] = {
        ...data,
        totalSize:
          index > 0 ? dataPoints[index - 1].totalSize + data.size : data.size,
        totalSizeUsd:
          index > 0
