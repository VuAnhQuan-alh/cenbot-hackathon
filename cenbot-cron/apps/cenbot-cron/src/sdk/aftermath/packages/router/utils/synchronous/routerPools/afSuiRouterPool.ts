...
class AfSuiRouterPool implements RouterPoolInterface {
  ...
  getTradeAmountOut = (inputs: {
    ...
    if (this.isStake(inputs)) {
      ...
      return BigInt(
        Math.floor(this.exchangeRate(inputs) * Number(inputs.coinInAmount)),
      );
    }
  ...
  getTradeAmountIn = (inputs: {
    ...
    if (this.isStake(inputs)) {
      ...
      const coinInAmount = BigInt(
        Math.ceil(Number(inputs.coinOutAmount) / this.exchangeRate(inputs)),
      );
  ...
  getUpdatedPoolAfterTrade = (inputs: {
    ...
    // @ts-ignore
    return new AfSuiRouterPool(Helpers.deepCopy(newPoolObject), this.network);
  };
